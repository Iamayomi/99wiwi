...
export function setupAdminRoutes(app: Express) {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const mostPlayedGame = await storage.getMostPlayedGame(
          startDate,
          endDate
        );

        // Get game distribution
        const gameDistribution = await storage.getGameDistribution(
          startDate,
          endDate
        );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch users", error: errorMessage });
      }
    }
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to search users", error: errorMessage });
      }
    }
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const updatedUser = await storage.banUser(
          userId,
          req.user!.id,
          banData.banReason
        );
        ...
        res
          .status(500)
          .json({ message: "Failed to ban user", error: errorMessage });
      }
    }
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to unban user", error: errorMessage });
      }
    }
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const { amount, reason } = adminCoinAdjustmentSchema
          .omit({ username: true })
          .parse(req.body);
    ...
    async (req, res) => {
      try {
          ...
          switch (bonusData.targetType) {
            ...
            case "active":
              targetUsers = targetUsers.filter(
                (user) => user.playCount >= 10 && user.playCount <= 100
              );
              break;
            case "veteran":
            ...
            case "custom":
              if (bonusData.filters) {
                ...
                if (minPlayCount !== undefined) {
                  targetUsers = targetUsers.filter(
                    (user) => user.playCount >= minPlayCount
                  );
                }
                if (maxPlayCount !== undefined) {
                  targetUsers = targetUsers.filter(
                    (user) => user.playCount <= maxPlayCount
                  );
                }
              }
              break;
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error";
        res.status(500).json({
          message: "Failed to process mass bonus",
          error: errorMessage,
        });
      }
    }
  ...
  app.get(
    "/api/admin/game-config/:gameType",
    authMiddleware,
    ownerMiddleware,
    async (req, res) => {
      try {
        ...
        // Validate game type
        if (
          !["slots", "dice", "crash", "roulette", "blackjack"].includes(
            gameType
          )
        ) {
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error";
        res.status(500).json({
          message: "Failed to fetch game config",
          error: errorMessage,
        });
      }
    }
    ...
    async (req, res) => {
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error";
        res.status(500).json({
          message: "Failed to update game config",
          error: errorMessage,
        });
      }
    }
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        const updatedSubscription = await storage.cancelSubscription(
          subscription.id
        );
    ...
    async (req, res) => {
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error";
        res.status(500).json({
          message: "Failed to fetch ban appeals",
          error: errorMessage,
        });
      }
    }
    ...
    async (req, res) => {
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error";
        res.status(500).json({
          message: "Failed to respond to ban appeal",
          error: errorMessage,
        });
      }
    }
