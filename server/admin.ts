...
export function setupAdminRoutes(app: Express) {
  ...
  app.post("/api/admin/users/:userId/adjust-balance", authMiddleware, ownerMiddleware, async (req, res) => {
    try {
      ...
      // Adjust user balance
      const updatedUser = await storage.adjustUserBalance(userId, amount, req.user!.id, reason);
    ...
    try {
      ...
      for (const user of targetUsers) {
        try {
          ...
          await storage.adjustUserBalance(user.id, bonusData.amount, adminId, bonusData.reason);
  ...
  app.patch("/api/admin/game-config", authMiddleware, ownerMiddleware, async (req, res) => {
    try {
      ...
      const updatedConfig = await storage.updateGameConfig(configData.gameType, configData.config);
  ...
  app.post("/api/admin/subscriptions/assign", authMiddleware, ownerMiddleware, async (req, res) => {
    try {
      ...
      const subscription = await storage.assignSubscriptionToUser(subscriptionData.userId, subscriptionData.tier, subscriptionData.durationMonths, req.user!.id, subscriptionData.reason);
  ...
  app.post("/api/admin/ban-appeals/:appealId/respond", authMiddleware, adminMiddleware, async (req, res) => {
    try {
      ...
      const updatedAppeal = await storage.respondToBanAppeal(appealId, req.user!.id, validatedData.status, validatedData.response);
