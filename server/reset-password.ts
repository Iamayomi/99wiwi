  ...
  try {
    ...
    if (!user) {
      return res.status(200).json({
        message:
          "If a user with that username exists, a password reset link has been sent to their email.",
      });
    }
    ...
    if (!user.email) {
      return res.status(400).json({
        message:
          "This account doesn't have an email address. Please contact support.",
      });
    }
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: user.email,
      subject: "Reset Your 99wiwi Password",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #5465FF;">99wiwi Password Reset</h2>
    ...
    console.log(
      "[PASSWORD RESET] Email sent successfully, data:",
