...
export class DatabaseStorage implements IStorage {
  ...
  async updateUserLastLogin(userId: number): Promise<User> {
    ...
    // Update with SQL query explicitly filtering on the user ID
    const [updatedUser] = await db.update(users).set({ lastLogin: new Date() }).where(eq(users.id, userId)).returning();
  ...
  async updateLoginStreak(userId: number, streak: number): Promise<User> {
    ...
    try {
      ...
      return await db.transaction(async (tx) => {
        ...
        const [lockedUser] = await tx.select().from(users).where(eq(users.id, userId)).for("update"); // SQL FOR UPDATE lock
  ...
  async getUserTransactions(userId: number, limit = 10): Promise<Transaction[]> {
    const userTransactions = await db.select().from(transactions).where(eq(transactions.userId, userId)).orderBy(desc(transactions.timestamp)).limit(limit);
  ...
  async incrementPlayCount(userId: number): Promise<User> {
    ...
    const [updatedUser] = await db.update(users).set({ playCount: newPlayCount }).where(eq(users.id, userId)).returning();
  ...
  async getAllUsers(limit = 50, offset = 0): Promise<User[]> {
    const allUsers = await db.select().from(users).orderBy(users.id).limit(limit).offset(offset);
  ...
  async getCoinTransactions(userId?: number, limit = 50): Promise<CoinTransaction[]> {
    let query = db.select().from(coinTransactions).orderBy(desc(coinTransactions.timestamp)).limit(limit);

    if (userId !== undefined) {
      return await db.select().from(coinTransactions).where(eq(coinTransactions.userId, userId)).orderBy(desc(coinTransactions.timestamp)).limit(limit);
    }
  ...
  async getUserLoginRewards(userId: number, limit = 30): Promise<LoginReward[]> {
    ...
    const rewards = await db.select().from(loginRewards).where(eq(loginRewards.userId, userId)).orderBy(desc(loginRewards.createdAt)).limit(limit);
  ...
  async getActiveUsersCount(startDate: Date, endDate: Date): Promise<number> {
    const result = await db
      ...
      .from(users)
      .where(and(sql`"last_login" IS NOT NULL`, sql`"last_login" >= ${startDate}`, sql`"last_login" <= ${endDate}`));
  ...
  async getCoinsSpent(startDate: Date, endDate: Date): Promise<number> {
    const result = await db
      ...
      .from(transactions)
      .where(and(sql`"game_type" IS NOT NULL`, sql`"timestamp" >= ${startDate}`, sql`"timestamp" <= ${endDate}`, sql`"is_win" = false`));
  ...
  async getCoinsEarned(startDate: Date, endDate: Date): Promise<number> {
    const result = await db
      ...
      .from(transactions)
      .where(and(sql`"game_type" IS NOT NULL`, sql`"timestamp" >= ${startDate}`, sql`"timestamp" <= ${endDate}`, sql`"is_win" = true`));
  ...
  async getMostPlayedGame(startDate: Date, endDate: Date): Promise<{ gameType: string; count: number }> {
    const result = await db
      ...
      .from(transactions)
      .where(and(sql`"game_type" IS NOT NULL`, sql`"timestamp" >= ${startDate}`, sql`"timestamp" <= ${endDate}`))
  ...
  async getDailyTransactions(): Promise<{ date: string; bets: number; wins: number }[]> {
    ...
    const betsResult = await db
      ...
      .from(transactions)
      .where(and(sql`"timestamp" >= ${startDate}`, sql`"game_type" IS NOT NULL`))
    ...
    const winsResult = await db
      ...
      .from(transactions)
      .where(and(sql`"timestamp" >= ${startDate}`, sql`"game_type" IS NOT NULL`, sql`"is_win" = true`))
      .groupBy(sql`DATE_TRUNC('day', "timestamp")::date`)
      .orderBy(sql`DATE_TRUNC('day', "timestamp")::date`);
  ...
  async getAnnouncements(includeExpired = false, userId?: number): Promise<any[]> {
    ...
    let filteredAnnouncements = includeExpired
      ...
      : this.announcements.filter((announcement) => {
          return announcement.isPinned || !announcement.expiresAt || new Date(announcement.expiresAt) > now;
        });
    ...
    if (userId !== undefined) {
      filteredAnnouncements = filteredAnnouncements.filter((announcement) => {
        ...
        const shouldInclude = !announcement.targetUserIds || (announcement.targetUserIds && announcement.targetUserIds.includes(userId));
  ...
  async getSupportTickets(status?: string, page = 1, limit = 20): Promise<any[]> {
    ...
    let results;
    if (status) {
      results = await queryBase.where(eq(supportTickets.status, status)).limit(limit).offset(offset);
    } else {
      results = await queryBase.limit(limit).offset(offset);
    }
    ...
    return [
      {
        ...
        features: ["300 daily coins", "Bronze VIP badge", "Basic support"],
      ...
      },
      {
        ...
        features: ["600 daily coins", "Silver VIP badge", "1.1x reward multiplier", "Ad-free experience", "Priority support"],
      ...
      },
      {
        ...
        features: ["1000 daily coins", "Gold VIP badge", "1.25x reward multiplier", "Ad-free experience", "Access to premium games", "Premium support"],
  ...
  async getBannedUsers(limit = 50, offset = 0): Promise<User[]> {
    const bannedUsers = await db.select().from(users).where(eq(users.isBanned, true)).orderBy(desc(users.bannedAt)).limit(limit).offset(offset);
  ...
  async getBanAppeals(status?: string, limit = 50, offset = 0): Promise<BanAppealType[]> {
    if (status) {
      return await db.select().from(banAppeals).where(eq(banAppeals.status, status)).orderBy(desc(banAppeals.createdAt)).limit(limit).offset(offset);
    } else {
      return await db.select().from(banAppeals).orderBy(desc(banAppeals.createdAt)).limit(limit).offset(offset);
    }
  }

  async getUserBanAppeal(userId: number): Promise<BanAppealType | undefined> {
    const [appeal] = await db.select().from(banAppeals).where(eq(banAppeals.userId, userId)).orderBy(desc(banAppeals.createdAt)).limit(1);
  ...
  async respondToBanAppeal(appealId: number, adminId: number, status: string, response: string): Promise<BanAppealType> {
    ...
    const [appeal] = await db.select().from(banAppeals).where(eq(banAppeals.id, appealId));
  ...
  async markPasswordResetTokenAsUsed(id: number): Promise<PasswordResetToken> {
    const [updatedToken] = await db.update(passwordResetTokens).set({ isUsed: true }).where(eq(passwordResetTokens.id, id)).returning();
  ...
  async updateUserPassword(userId: number, newPassword: string): Promise<User> {
    const [updatedUser] = await db.update(users).set({ password: newPassword }).where(eq(users.id, userId)).returning();
  ...
  async updateUserEmail(userId: number, email: string): Promise<User> {
    const [updatedUser] = await db.update(users).set({ email }).where(eq(users.id, userId)).returning();
